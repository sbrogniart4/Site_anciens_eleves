...
ECHO'<H1>MODIFICATION DU TEXTE NEWS</H1>';
echo'</div>';
...
echo"<div id='bande_inscription'>";
	...
	$requete=$bdd->query('SELECT banderole,texte FROM news WHERE id=1');
	
	while ($ligne = $requete->fetch())	
	{

	echo'<h1>la banderole : '.$ligne["banderole"].'</h1>' ;					
echo'</div>';

echo 'le texte : '.$ligne["texte"];
	 ...
	 if(isset($_POST["updt1"]))
			...
			echo "<input type='submit' name='conf1' value='Confirmer' />";
							...
							if(isset($_POST["conf1"]))
							{
							 ...
							 $req = $bdd->prepare("UPDATE news SET banderole= :banderole ");
						$req->execute(array(
				...
				if(isset($_POST["conf2"]))
							{
							 ...
							 $req3 = $bdd->prepare("UPDATE news SET texte= :texte ");
						$req3->execute(array(
